...
describe("Products details", () => {
    ...
    it("renders elements of the component for an available product", async () => {
        render(
            <ProductsDetailsComponent
                id={"1"}
                name={"brouette"}
                description={"Description de la brouette"}
                price={10}
                stock={5}
                picture={"image.jpg"}
                available={true}
        ...
        expect(
            screen.getByRole("heading", { name: "brouette" }),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/Description de la brouette/i),
        ).toBeInTheDocument();
    ...
    describe("When user clicks on the reservation button", () => {
        it("should open a modal showing a preview of the cart", async () => {
            ...
            expect(
                screen.queryByText(
                    /Le produit brouette a été ajouté au panier/i,
                ),
