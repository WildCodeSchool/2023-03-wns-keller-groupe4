...
const ProductsListPage = () => { 
  ...
  const {error: Qcategory_error, loading: Qcategory_loading, data: Qcategory }  = useQuery(
  ...
  if(Qcategory_error && category_name !== "") {
    return (
      <div className="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4 m-5" role="alert">
        ...
        <p>La catégorie de produits <span className="bg-gray-100 text-gray-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">{ category_name }</span> n'existe pas. Veuillez réessayer avec une autre catégorie.</p>
      </div>
    );
  }
  ...
  if (products.getProductsByCategory.length === 0) {
    return (
      <div className="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4 m-5" role="alert">
        ...
        <p>La catégorie <span className="bg-gray-100 text-gray-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300">{ category_name }</span> ne contient aucun produits pour le moment.</p>
      </div>
    );
  } else {
    return (
